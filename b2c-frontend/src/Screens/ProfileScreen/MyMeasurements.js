...
import BodyPixMeasurement from '../../Components/Bodypix/BodyPixMeasurement';
...
const initForm2 = {sleeveLength : 10, shoulderWidth : null, chestCircumference : null, hips : null, legLength: null, ptp: null, torsoLength : null, thighCircumference : null, kneeCircumference : null, calfCircumference : null}
...
export default function MyMeasurements() {
  ...
  const [isBodyPixOpen, setIsBodyPixOpen] = React.useState(false);

  function handleBodyPixSubmit(measurements) {
    setIsBodyPixOpen(false);
    console.log("BODYPIX SUBMIT");
    let newForm = form;
    Object.entries(measurements).forEach(([k,v],i) => {
      console.log(k, ": ", v);
      if(Object.keys(initForm).includes(k) && v) {
        newForm[k] = v;
        //setForm({ ...form, [k]:v });
      }
    })
  ...
  return (
    ...
    <Container sx={{mb : 6}}>
      ...
      <Box component='form'>
        <Box sx={{display : 'flex', justifyContent : 'space-between'}}>
          ...
          <CustomButton variant='contained' color='secondary' onClick={() => setIsBodyPixOpen(true)}>Predict my measurements</CustomButton>
        </Box>
    ...
    <BodyPixMeasurement open={isBodyPixOpen} onSubmit={handleBodyPixSubmit} onClose={() => setIsBodyPixOpen(false)}/>
    </>
  )
}
