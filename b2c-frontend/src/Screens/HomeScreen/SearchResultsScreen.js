...
const SearchResultsScreen = ({ props }) => {
  ...
  const getResults = async () => {
    ...
    try {
      if (id[0] != 0 && keyword !== '') {
        console.log('both')
        await listingApi.getListingsByCategoryAndKeyword(id, keyword).then((arr) => setListings(arr.data))
      } else if (id[0] == 0 && keyword !== '') {
        console.log('key')
        await listingApi.getListingsByKeyword(keyword).then((arr) => setListings(arr.data))
      } else {
        console.log('cat')
        await listingApi.getListingsByCategory(id).then((arr) => setListings(arr.data))
      }
    } catch (error) {
      console.log(error);
      setListings([]);
  ...
  const refreshResults = async (newArr, newKeyword) => {
    try {
      ...
      if (newArr.length > 0 && newKeyword !== '') {
        console.log('add 1');
        await listingApi.getListingsByCategoryAndKeyword(catId, keywordSearch).then((arr) => {
          setListings(arr.data);
        })
      } else if (newArr.length > 0 && newKeyword === '') {
        console.log('add 2');
        await listingApi.getListingsByCategory(catId).then((arr) => {
          setListings(arr.data);
        });
      } else if (newArr.length === 0 && newKeyword !== '') {
        console.log('add 3');
        await listingApi.getListingsByKeyword(newKeyword).then((arr) => {
          setListings(arr.data);
        });
      } else {
        console.log('add 4');
        await listingApi.getAllListings().then((arr) => {
          setListings(arr.data);
        });
      }
    } catch (error) {
      ...
      >
        <Container maxWidth={false}>
          ...
          >
            ...
            >
              <div className='homepage_title' style={{ fontWeight: "bold", }}>Refashion ideas for...</div>
            </Grid>
            <Grid
              item
              lg={12}
                      ...
                      <Box>
                        <Card sx={{ mb: 1, position: 'relative' }}>
                          <Link to={`/listing/${list.id}`}>
                            <ImageListItem key={list.id} style={{ minHeight: '22vw', maxHeight: '22vw', maxWidth: '22vw', minWidth: '22vw', overflow: 'hidden' }} >
