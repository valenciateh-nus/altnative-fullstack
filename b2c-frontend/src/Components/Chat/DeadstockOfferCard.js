...
export default function DeadstockOfferCard({handleSubmit, isAddOn = false}) {
  ...
  const icons = {
    ...
    const fields = [{name : 'proposedCompletionDate', label : 'Timeline', unit :  ''}, {name : 'price', label : 'Offer Price', unit : "SGD "}, {name : 'description', label : 'Descriptions', unit: ''}]
      return (
          <>
                ...
                <Box component="form" onSubmit={handleSubmit} sx={{display : 'flex', justifyContent: 'space-between', flexDirection : 'column'}}>
                    ...
                    <Accordion sx={{boxShadow : 'none', backgroundColor : "#FFFAF0"}}>
                        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
                            ...
                            <Typography sx={{paddingLeft: 1,whiteSpace: 'nowrap',overflow: 'hidden',textOverflow: 'ellipsis', maxWidth : '140px'}}>
                                {offerForm.price === "" || offerForm.price === null ? 'Offer Price' : `SGD ${offerForm.price}`}
                            </Typography>
                        </AccordionSummary>
                        <AccordionDetails sx={{paddingBottom : 0}}>
                        <TextField size ='small' variant="outlined" id='price' 
                            name='price' color="secondary" label='Offer Price' value={offerForm.price} onChange={handleFormChange}
                            type='number' required error={offerForm.price <= 0}
                        />
                        </AccordionDetails>
                    </Accordion>
                    <Accordion sx={{boxShadow : 'none', backgroundColor : "#FFFAF0"}}>
                    ...
                    <Box sx={{display: 'flex', flexGrow : 1, justifyContent : 'center', paddingLeft: 5, paddingRight: 5}}>
                        ...
                        : <CustomButton variant='contained' color='secondary' fullWidth onClick={handleOnSubmit} disabled={offerForm.proposedCompletionDate && offerForm.proposedCompletionDate.isValid() && moment(new Date()).isSameOrBefore(offerForm.proposedCompletionDate) && offerForm.description && Number(offerForm.price) > 0 ? false : true}
                        >
                        Send Offer
                        </CustomButton>
                        }
