import { Box, Button, Card, Chip, CircularProgress, Grid, ImageList, ImageListItem, ImageListItemBar, MenuItem, Select, SvgIcon, TextField, ToggleButton, ToggleButtonGroup, Typography, useMediaQuery } from '@mui/material';
...
const ProfileRequest = ({ id = 0, username }) => {
  ...
  React.useEffect(() => {
    if (id == 0) {
      getMyRequestsByKeyword();
    } else {
      getPublishedListingsByKeyword();
    }
  }, [searchValue])


  async function getMyRequestsByKeyword () {
    try {
  ...
  async function getPublishedListingsByKeyword () {
    try {
  ...
  const getPublishedListings = async () => {
    try {
      ...
      //   setListings(array);
      // })
      await indexApi.getRequestByUsername(username).then((arr) => {
        const array = arr.data;
        array.sort(function (a, b) {
          // Turn your strings into dates, and then subtract them
          // to get a value that is either negative, positive, or zero.
          ...
          (notHidden ? (
            ...
            {id == 0 && 
              ...
              {Array.from(listings).map((list) => (
                <Link to={`/requestDetails/${list.id}`}>
                  {console.log(list)}
          ...
          <>
            <Grid item xs={12} sx={{ display: 'block'}}>
              <Box sx={{ display: 'flex', justifyContent: "center", alignContent: 'center', marginTop: 5 }}>
                <Typography>
                No <Typography fontWeight={600} display="inline"> {toggleValue.toLowerCase()} </Typography> requests found.
                </Typography>
              </Box>
