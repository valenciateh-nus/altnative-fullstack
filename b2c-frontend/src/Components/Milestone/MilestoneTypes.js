...
export function getMilestoneTitle(type, description, isRefashionee, otherUser) {
    switch (type) {
        case ORDER_CREATED:
        case ORDER_STARTED:
            return (
        ...
        case ADD_ON_OFFER_DECLINED:
            return (
                <Typography>
                    {otherUser?.roles?.includes("USER_BUSINESS") ? (
                      `Refashionee has rejected the Additional order offer. ${description ? `Reason : ${description}` : ''}`
                    ) : (
                        `Business has rejected the Additional order offer. ${description ? `Reason : ${description}` : ''}`
                    )}
                </Typography>
            )
        ...
        case PAYMENT:
            return (
                <Typography color={'secondary.main'} fontWeight='fontWeightBold'>
                    {otherUser?.roles?.includes("USER_BUSINESS") ? `Business has paid SGD ${description} for the order.` : `Refashionee has paid SGD ${description} for the order.`}
                </Typography>
            )
        case PROGRESS_UPDATE_REQUEST:
            return (
                <Typography>
                    <Typography display={'inline'} color={'secondary.main'} fontWeight='fontWeightBold'>{otherUser?.roles?.includes("USER_BUSINESS") ? "The business" : "The refashionee"} has requested for progress pictures. </Typography>
                    Please upload soon.
                </Typography>
            )
        ...
        case FINAL_APPROVAL_OK:
            return (
                <Typography>
                    {isRefashionee ? "You have" : (otherUser?.roles?.includes("USER_BUSINESS") ? "The business has" : "The refashionee has")} approved the final pictures! Awaiting for courier arrangement.
                </Typography>
            )
        case FINAL_APPROVAL_REJECTED:
            return (
                <Typography>
                    {isRefashionee ? "You have" : (otherUser?.roles?.includes("USER_BUSINESS") ? "The business has" : "The refashionee has")} rejected the final pictures. Reason: {description}
                </Typography>
            )
