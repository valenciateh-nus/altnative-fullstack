...
const Requests = () => {
  ...
  const refreshResults = async (val, search) => {
    try {
      if (val === 'ALL') {
        await orderApi.getRefashioneeOrdersByKeyword(search).then((arr) => {
          const array = arr.data;
          array.sort(function(a,b){
            return new Date(b.orderTime) - new Date(a.orderTime);
          })
        ...
        await orderApi.getRefashioneeOrdersByStatusKeyword(val, search).then((arr) => {
          const array = arr.data;
          array.sort(function(a,b){
            return new Date(b.orderTime) - new Date(a.orderTime);
          })
            ...
            <>
              {orderList.length === 0 ? (
                ...
                (Array.from(orderList).map((val) => (
                    <OrderList title={val.offerTitle} name={val.refashionerUsername} status={val.orderStatus} id={val.id} chat={val.chatAlternateId}/>
                )))
              }
            </>
          ) : (
