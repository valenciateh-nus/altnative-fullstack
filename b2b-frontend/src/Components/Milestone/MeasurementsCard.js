...
export default function MeasurementsCard({milestone = {},  editMilestone, index, isRefashionerPOV, handleRefresh}) {
                ...
                <Box sx={{display : 'flex', flexGrow: 1, justifyContent : 'space-between', alignItems : 'center', marginBottom : '8px', marginRight : 2, }}>
                    ...
                    {!remarksJSON.isResponse && 
                        <CustomButton variant='contained' color="secondary" type="submit"
                            disabled={Object.values(measurementsForm).filter((field) => Number(field) <= 0).length === 0 ? false : true} 
                        >
                            Submit
                        </CustomButton>
                    }
