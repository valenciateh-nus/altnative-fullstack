...
export default function ChatSection({selectedChat = {messages: []}, selectedTopic = {}, otherUser, handleOnBack, setSelectedChat, sendStompMessage, handleDeleteChat, handleRefresh, innerLoading = false}) {
    ...
    return (
        <>
            <Grid item container columns = {14} xs={expandTopBar? 1 : 1.8} id='messagesTopBar' sx={{ justifyContent: 'center', alignItems: 'center'}} direction='row'>
                ...
                <Grid item xs={12}>
                    <Box sx={{ flexDirection: 'row', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                        <Box sx= {{cursor : 'pointer', display : 'flex', flexDirection : 'row', justifyContent : 'center', alignItems : 'center'}} onClick={() => history('/profile/' + otherUser.username)}>
                ...
                <Grid item xs={1} className={styles.iconContainer}>
                <ChatMoreMenu otherUser={otherUser} topic = {selectedTopic} 
                    isMpl = {selectedTopic?.marketplaceListingStatus} 
                    canOffer={(selectedTopic?.id && selectedTopic.available 
                    && ( (selectedTopic.refashionee && selectedTopic.refashionee.username !== currUser.sub) || (selectedTopic.refashioner?.username === currUser.sub) || (selectedTopic.appUser?.username === currUser.sub) ))} 
                    canDelete={(selectedChat.orders?.length === 0 || !selectedChat.topic || !selectedChat.orders) && selectedChat.messages?.length > 0 } 
                    chatId = {selectedChat.chatAlternateId} handleDeleteChat={handleDeleteChat} 
