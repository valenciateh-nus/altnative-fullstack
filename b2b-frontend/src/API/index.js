...
export const signUpAsBusiness = (formData) => API.post('/api/v1/business/register', formData);
...
export const getRequestsByStatus = (status, page) => API.get(`/api/v1/myProjectRequests/search/?statuses=${status}&page=${page || 0}`);
...
export const getRequestsByStatusKeyword = (status, keyword) => API.get(`/api/v1/myProjectRequests/search/?statuses=${status}&keyword=${keyword}`);

export const getAllRequests = (page) => API.get(`/api/v1/projectRequests/search/?page=${page || 0}`);
...
export const getRequestByUsername = (username, page) => API.get(`/api/v1/projectRequests/refashionee/${username}/?page=${page || 0}`);

export const getPublishedRequests = (username) => API.get(`/api/v1/projectRequests/username/${username}`);

export const getPublishedRequestsByKeyword = (username, keyword) => API.get(`/api/v1/projectRequests/username/${username}/?keyword=${keyword}`);

export const getAllRequestsByKeyword = (keyword) => API.get(`/api/v1/allProjectRequests/search/?keyword=${keyword}`);

export const getAllRequestsByCategoryKeyword = (cId, keyword) => API.get(`/api/v1/allProjectRequests/search/?keyword=${keyword}&cIds=${cId}`);
...
export async function apiWrapper(apiCall, errMessage = "", dispatchError) {
    try {
        ...
        if(dispatchError) {
            store.dispatch({type : ERROR, data: (errMessage + (errMessage && ", ") + (error?.response?.data ? error.response.data : "Unknown error occured."))})
        }
    }
    return null;
}