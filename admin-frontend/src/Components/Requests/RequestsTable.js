...
import RequestsMoreMenu from './RequestsMoreMenu';
import Collapse from '@mui/material/Collapse';
import KeyboardArrowDownIcon from '@mui/icons-material/KeyboardArrowDown';
import KeyboardArrowUpIcon from '@mui/icons-material/KeyboardArrowUp';
import RefashionerApplicationModal from './RefashionerApplicationModal';
import { useNavigate } from 'react-router-dom';
import { BANK_REQ, BUS_REQ, DISPUTE_REQ, REFASHIONER_REQ, WITHDRAW_REQ, SWAP_REQ } from '../../Screens/RequestsManagementScreen';

import BankAccountRequestModal from './BankAccountRequestModal';
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <TextField placeholder="Search by applicant or id"
          variant="outlined"
          fullWidth
          value = {searchValue}
...
function NormalRow({isItemSelected, row, labelId, rowNames, handleClick, handleView, index}) {
  return (
    ...
    >
      ...
      <TableCell>
        <CustomButton variant='contained' color='secondary' onClick={handleView}>View Request</CustomButton>
      </TableCell>
    </TableRow>
  )
}
...
export default function EnhancedTable({rows, rowNames, handleRefresh, type}) {
  ...
  const [isBARModal, setIsBARModal] = React.useState(false);
  const [isWRModal, setIsWRModal] = React.useState(false);
  const [isSRModal, setIsSRModal] = React.useState(false);
  ...
  function handleView(someObject) {
    console.log("VIEW CLICKED");
    if(type === DISPUTE_REQ) {
      navigate(`/dispute/${someObject.id}`);
    } else if (type === BUS_REQ) {

    } else if (type === REFASHIONER_REQ) {
      handleOpenRAModal(someObject);
    } else if (type === BANK_REQ) {
      handleOpenBARModal(someObject);
    } else if (type === WITHDRAW_REQ) {
      handleOpenWRModal(someObject);
    } else if (type == SWAP_REQ) {
      handleOpenSRModal(someObject);
    }
  }
  ...
  function handleOpenBARModal(bar) {
    setSelectedBAR(bar);
    setIsBARModal(true);
  }
  ...
  function handleOpenWRModal(wr) {
    setSelectedWR(wr);
    setIsWRModal(true);
  }

  function handleCloseWRModal() {
    setSelectedWR({});
    setIsWRModal(false);
  }

  function handleOpenSRModal(sr) {
    setSelectedSR(sr);
    setIsSRModal(true);
  }

  function handleCloseSRModal() {
    setSelectedSR({});
    setIsSRModal(false);
  }
  ...
  return (
            ...
            <TableBody>
              ...
              {stableSort(rows, getComparator(order, orderBy))
                .filter(row => {
                  ...
                  return row.user?.username?.toLowerCase().includes(searchValue) || String(row.id) === searchValue || searchValue === null || searchValue === ''
                })
                ...
                .map((row, index) => {
                  ...
                  return (
                    <NormalRow row = {row} isItemSelected = {isItemSelected} index={index} labelId = {labelId} rowNames = {rowNames} handleClick = {handleClick} handleView = {() => handleView(row)}/>
                  );
                })}
              {emptyRows > 0 && (
                <TableRow
    ...
    <Modal open={isBARModal} onClose = {handleCloseBARModal}>
      <Paper sx={{position: 'absolute', top: '50%', left: '50%',transform: 'translate(-50%, -50%)', width: '80%', height: '80%', padding: 4, overflow: 'scroll'}}>
        <BankAccountRequestModal form={selectedBAR} handleRefresh={handleRefresh}/>
      </Paper>
    </Modal>
    <Modal open={isWRModal} onClose = {handleCloseWRModal}>
      <Paper sx={{position: 'absolute', top: '50%', left: '50%',transform: 'translate(-50%, -50%)', width: '80%', height: '80%', padding: 4, overflow: 'scroll'}}>
        <WithdrawRequestModal wr={selectedWR} handleRefresh={handleRefresh}/>
      </Paper>
    </Modal>
    <Modal open={isSRModal} onClose = {handleCloseSRModal}>
      <Paper sx={{position: 'absolute', top: '50%', left: '50%',transform: 'translate(-50%, -50%)', width: '80%', height: '80%', padding: 4, overflow: 'scroll'}}>
        <SwapRequestModal sr={selectedSR} handleRefresh={handleRefresh}/>
      </Paper>
    </Modal>
    </>
  );
