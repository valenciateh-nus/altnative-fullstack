  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <TextField placeholder="Filter by amount, offer id, or order id"
          variant="outlined"
          fullWidth
          value = {searchValue}
  ...
  return (
    ...
    >
      {rowNames.map((name, idx) => 
        ...
        >
          {name.date ? (moment(row[name.id]).isValid() ? moment(row[name.id]).format('DD MMM yyyy') : 'NIL') : (name.object ? ( row[name.id]?.id ? row[name.id][name.objectField] : '-') : String(row[name.id]))}
        </TableCell> 
        : <TableCell> {name.date ? (moment(row[name.id]).isValid() ? moment(row[name.id]).format('DD MMM yyyy') : '-') : ( name.object ? ( row[name.id]?.id ? row[name.id][name.objectField] : '-') : String(row[name.id]))}</TableCell>
      )}
      <TableCell>
        <CustomButton variant='contained' color='secondary' onClick={handleView}>View Transaction</CustomButton>
      </TableCell>
    </TableRow>
  )
}
...
export default function EnhancedTable({rows, rowNames, handleRefresh, type}) {
  ...
  function handleView(txn) {
    handleOpenRAModal(txn);
  }

  function handleOpenRAModal(txn) {
    setSelectedRA(txn);
    setIsRAModal(true);
  }
  ...
  return (
          ...
          >
            ...
            <TableBody>
              ...
              {stableSort(rows, getComparator(order, orderBy))
                .filter(row => String(row.id) === searchValue || String(row.order2?.id) === searchValue || String(row.amount) === searchValue || String(row.offerId) === searchValue || searchValue === '')
    ...
    <Modal open={isRAModal} onClose = {handleCloseRAModal}>
      <Paper sx={{position: 'absolute', top: '50%', left: '50%',transform: 'translate(-50%, -50%)', width: '50%', height: '80%', padding: 4, overflow: 'scroll'}}>
        <TransactionsModal transaction={selectedRA}/>
      </Paper>
    </Modal>
    </>
  );
