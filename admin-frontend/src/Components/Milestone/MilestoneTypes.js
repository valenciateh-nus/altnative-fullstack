...
export function getMilestoneTitle(type, description, isRefashionee) {
    switch (type) {
        case ORDER_CREATED:
        case ORDER_STARTED:
            return (
        ...
        case MATERIAL_LOCATION_PENDING:
            return (
                <Typography color={!description ? 'secondary.main' : 'inherit'} fontWeight={!description ? 'fontWeightBold' : 400}>
                    {isRefashionee ? 'Pickup location required' : 'Dropoff location required'}
                </Typography>
            )
        ...
        case ADD_ON_OFFER_MADE:
            return (
                <Typography>
                    Refashioner has sent an Additional order offer
                </Typography>
            )
        case ADD_ON_OFFER_DECLINED:
            return (
                <Typography>
                    Refashionee has rejected the Additional order offer. {description ? `Reason : ${description}` : ''}
                </Typography>
            )
        ...
        case PAYMENT:
            return (
                <Typography color={'secondary.main'} fontWeight='fontWeightBold'>
                    {`Refashionee has paid SGD ${description} for the order.`}
                </Typography>
            )
        case PROGRESS_UPDATE_REQUEST:
            return (
                <Typography>
                    <Typography display={'inline'} color={'secondary.main'} fontWeight='fontWeightBold'>Refashionee has requested for progress pictures. </Typography>
                    Please upload soon.
                </Typography>
            )
        ...
        case FINAL_APPROVAL_PENDING:
            return (
                <Typography>
                    {!isRefashionee ? <>You have sent the <Typography display={'inline'} color={'secondary.main'} fontWeight='fontWeightBold'>final pictures</Typography>. Pending approval from the refashionee.</>
                        : <>Refashioner has sent the <Typography fontWeight='fontWeightBold' display={'inline'} color={'secondary.main'}>Final pictures</Typography>. Pending your approval.</>}
                </Typography>
            )
        case FINAL_APPROVAL_OK:
            return (
                <Typography>
                    {isRefashionee ? "You have" : "The refashionee has"} approved the final pictures! Awaiting for courier arrangement.
                </Typography>
            )
        case FINAL_APPROVAL_REJECTED:
            return (
                <Typography>
                    {isRefashionee ? "You" : "The refashionee"} has rejected the final pictures. Reason: {description}
                </Typography>
            )
        ...
        case DISPUTE_REQUEST_REFUND_PENDING:
            return (
                <Typography>
                    A refund is <Typography color={'secondary.main'} display={'inline'} fontWeight='fontWeightBold'> processing </Typography>:
                </Typography>
            )
        case DISPUTE_REQUEST_REJECTED:
            return (
                <Typography>
                    A <Typography color={'secondary.main'} display={'inline'} fontWeight='fontWeightBold'> Dispute Request </Typography> has been rejected:
                </Typography>
            )
        case DISPUTE_REQUEST_COMPLETED:
            return (
                <Typography>
                    A <Typography color={'secondary.main'} display={'inline'} fontWeight='fontWeightBold'> Dispute Request </Typography> has been completed:
                </Typography>
            )   
        case DISPUTE_REQUEST_PENDING_ADMIN_REVIEW:
            return (
                <Typography>
                    A <Typography color={'secondary.main'} display={'inline'} fontWeight='fontWeightBold'> Dispute Request </Typography> is currently being reviewed by an admin:
                </Typography>
            )
        case DISPUTE_REQUEST_ACCEPTED:
            return (
                <Typography>
                    A <Typography color={'secondary.main'} display={'inline'} fontWeight='fontWeightBold'> Dispute Request </Typography> has been approved:
                </Typography>
            )   
            case DISPUTE_REQUEST_PENDING_ADMIN_REVIEW:
                return (
