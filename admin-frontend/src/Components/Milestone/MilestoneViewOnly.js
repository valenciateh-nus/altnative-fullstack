...
export default function MilestoneViewOnly({ orderId, isRefashionerPOV }) {
  ...
  async function loadMilestones() {
    ...
    const res = await apiWrapper(MilestoneApi.retrieveMilestones(orderId), 'Could not fetch milestones. Please try again later.', true);
  ...
  function milestoneTypeInput(milestoneType, milestone, isLast, index) {
    switch (milestoneType) {
      case MilestoneTypes.MEASUREMENTS_REQUESTED:
        return <MeasurementsCard milestone={milestone} editMilestone={editMilestone} index={index} isRefashionerPOV={isRefashionerPOV} handleRefresh={loadMilestones} viewOnly={true}/>
      case MilestoneTypes.ARRANGE_FOR_COURIER:
        return <Delivery deliveryId={milestone.remarks} handleRefresh={loadMilestones} viewOnly={true} />
      ...
      case MilestoneTypes.ADD_ON_ORDER_COMPLETE:
        return <OfferChatCard offerId={milestone.offerId} isLast={false} showButtons={!isRefashionerPOV} orderId={orderId} handleRejection={loadMilestones} viewOnly = {true} />
      case MilestoneTypes.PAYMENT:
        return
      case MilestoneTypes.FINAL_APPROVAL_PENDING:
        return <FinalApproval milestone={milestone} editAndCreate={editAndCreate} handleRefresh={loadMilestones} isRefashionerPOV={isRefashionerPOV} viewOnly = {true}/>
      case MilestoneTypes.DISPUTE_REQUEST_PENDING_REVIEW:
      case MilestoneTypes.DISPUTE_REQUEST_REJECTED:
      case MilestoneTypes.DISPUTE_REQUEST_PENDING_ADMIN_REVIEW:
      case MilestoneTypes.DISPUTE_REQUEST_REFUND_PENDING:
      case MilestoneTypes.DISPUTE_REQUEST_ACCEPTED: 
      case MilestoneTypes.DISPUTE_REQUEST_COMPLETED: 
      case MilestoneTypes.DISPUTE_REQUEST_PENDING_REVIEW:
          return <DisputeListings disputeId={milestone.disputeId} />
      case MilestoneTypes.DISPUTE_REQUEST_REFUND_PENDING:
        return <DisputeRefundForm disputeId={milestone.disputeId} viewOnly={true} />
      default:
        return;
    }
  }
          ...
          {milestones.slice().reverse().map((milestone, index) => (
            <TimelineItem key={index}>
              ...
              <TimelineContent sx={{ paddingTop: 0, paddingBottom: '30px', paddingRight: 0, }}>
                {getMilestoneTitle(milestone.milestoneEnum, milestone.remarks, !isRefashionerPOV)}
