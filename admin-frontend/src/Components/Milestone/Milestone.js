...
import { Button, CircularProgress, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Grid, IconButton, ImageList, ImageListItem, Typography, } from "@mui/material";
...
export default function Milestone({ orderId, isRefashionerPOV, handleBack, otherUser }) {
  ...
  function milestoneTypeInput(milestoneType, milestone, isLast, index) {
    switch (milestoneType) {
      ...
      case MilestoneTypes.FINAL_APPROVAL_PENDING:
        return <FinalApproval milestone={milestone} editAndCreate={editAndCreate} handleRefresh={loadMilestones} isRefashionerPOV={isRefashionerPOV} />
  ...
  return (
    !isLoading ? (milestones.length > 0 && <>
      <Grid item container xs={1} justifyContent={'space-between'}>
        ...
        <MilestoneMenubar isRefashionerPOV={isRefashionerPOV} orderId={orderId} handleRefresh={loadMilestones} createMilestone={createMilestone} milestoneTypes={milestones.map(({ milestoneEnum }) => milestoneEnum)} />
      </Grid>
