...
export default function ChatScreen() {
  ...
  async function loadMessages() {
    ...
    if(!chatLoaded.has(selectedChat)) {
      ...
      const res = await apiWrapper(getChatMessages(selectedChat),'',false);
      if(res) {
...
console.log("CURRENT CHAT: ", chats.get(selectedChat));
  ...
  async function handleRefresh(chatId) {
    ...
    const chatRes = await apiWrapper(getChat(chatId),'',false);
    if(chatRes) {
      console.log(chatRes);
    }
  ...
  },[params.chatId, searchParams.get('user2')])
    ...
    !isLoading && currUser?.sub ? <Box className = {styles.root} id = 'chatScreenRoot'>
      ...
      <Grid container columns = {14} id = 'chatScreenGridContainer' sx = {{height : '100%'}}>
        ...
        <>
          ...
          !innerLoading ?
          <Grid container item md = {10} xs = {14} id = 'messagesContainer' direction='column' sx = {{height : '100%', overflowX: 'hidden'}} wrap={"nowrap"}>
          ...
          :<>{!mobile && 
            <Grid container item md = {10} id = 'messagesContainer' direction='column' sx = {{justifyContent : 'center', alignItems : 'center'}}>
              <Button
              variant="contained"
              disabled = {true}
