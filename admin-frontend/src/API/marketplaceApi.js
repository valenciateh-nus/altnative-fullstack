...
export const retrieveAllMarketplaceListingsByUsername = (username) => API.get(`/api/v1/marketplaceListings/user/${username}`)

export const searchAllDeadstocks = (cIds, keyword) => API.get(`/api/v1/allDeadstocks/search?${cIds.length > 0 ? `cIds=${cIds}&` : ''}${keyword.length > 0 ? `keyword=${keyword}` : ''}`);

export const searchAvailableDeadstocks = (cIds, keyword) => API.get(`/api/v1/availableDeadstocks/search?${cIds.length > 0 ? `cIds=${cIds}&` : ''}${keyword.length > 0 ? `keyword=${keyword}` : ''}`);