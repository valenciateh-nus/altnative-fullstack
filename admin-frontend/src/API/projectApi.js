...
export const searchBusinessRequests = (cIds,keyword) => API.get(`/api/v1/allBusinessRequests/search?${cIds.length > 0 ? `cIds=${cIds}&` : ''}${keyword.length > 0 ? `keyword=${keyword}` : ''}`);

export const searchAvailableBusinessRequests = (cIds,keyword) => API.get(`/api/v1/allAvailableBusinessRequests/search?${cIds.length > 0 ? `cIds=${cIds}&` : ''}${keyword.length > 0 ? `keyword=${keyword}` : ''}`);
